...
import { Boxes, Building2, CalendarDays, CheckCircle2, CreditCard, FileKey2, ImagePlus, MapPin, RadioTower, Save, ShieldCheck, SlidersHorizontal, Trash2, UsersRound } from '@lucide/vue'
...
type CompanyTab = 'identificacao' | 'endereco' | 'logo'
const activeTab = ref<SettingsTab>('empresa')
const activeCompanyTab = ref<CompanyTab>('identificacao')
...
const companyTabs: Array<{ id: CompanyTab; label: string; icon: Component }> = [
  { id: 'identificacao', label: 'Identificação', icon: Building2 },
  { id: 'endereco', label: 'Endereço fiscal', icon: MapPin },
...
async function save() {
  ...
  try {
    ...
    if (activeTab.value === 'empresa') showCompanyErrorTab(parsed.errors)
    toast.show('error', parsed.message)
...
function showCompanyErrorTab(fieldErrors: Record<string, string[]>) {
  const keys = Object.keys(fieldErrors)
  if (keys.some((key) => ['cep', 'uf', 'logradouro', 'numero', 'complemento', 'bairro', 'municipio', 'codigo_municipio_ibge'].includes(key))) activeCompanyTab.value = 'endereco'
  else if (keys.some((key) => ['logo'].includes(key))) activeCompanyTab.value = 'logo'
  else activeCompanyTab.value = 'identificacao'
}
onMounted(load)
</script>
    ...
    <div class="settings-layout">
      ...
      <section class="panel settings-panel" :class="{ 'is-loading': loading }">
        <div v-if="activeTab === 'empresa'" class="settings-section"><header><span class="settings-icon"><Building2 :size="21" /></span><div><h2>Dados da empresa</h2><p>Informações que identificam o emitente nos documentos fiscais.</p></div></header>
          <nav class="settings-subtabs" aria-label="Seções dos dados da empresa"><button v-for="tab in companyTabs" :key="tab.id" :class="{ active: activeCompanyTab === tab.id }" @click="activeCompanyTab = tab.id"><component :is="tab.icon" :size="16" />{{ tab.label }}</button></nav>
          <div v-if="activeCompanyTab === 'identificacao'" class="settings-form"><label class="field-span-2"><span>Razão social *</span><input v-model="form.razao_social" placeholder="Razão social da empresa"><small v-if="errors.razao_social">{{ errors.razao_social[0] }}</small></label><label><span>Nome fantasia</span><input v-model="form.nome_fantasia" placeholder="Nome usado comercialmente"></label><label><span>CNPJ *</span><input :value="form.cnpj" inputmode="numeric" placeholder="00.000.000/0000-00" @input="form.cnpj = digits(($event.target as HTMLInputElement).value)"><small v-if="errors.cnpj">{{ errors.cnpj[0] }}</small></label><label><span>Inscrição estadual</span><input v-model="form.inscricao_estadual" :disabled="form.inscricao_estadual_isento" placeholder="Inscrição estadual"></label><label class="checkbox-field"><input v-model="form.inscricao_estadual_isento" type="checkbox"><span>Contribuinte isento</span></label><label><span>Inscrição municipal</span><input v-model="form.inscricao_municipal" placeholder="Inscrição municipal"></label><label><span>CNAE</span><input v-model="form.cnae" inputmode="numeric" placeholder="Código CNAE"></label><label><span>Telefone</span><input v-model="form.telefone" placeholder="(00) 0000-0000"></label><label><span>Celular</span><input v-model="form.celular" placeholder="(00) 00000-0000"></label><label class="field-span-2"><span>E-mail</span><input v-model="form.email" type="email" placeholder="empresa@exemplo.com.br"></label></div>
          <div v-else-if="activeCompanyTab === 'endereco'" class="settings-form"><label><span>CEP *</span><input v-model="form.cep" inputmode="numeric" placeholder="00000-000"><small v-if="errors.cep">{{ errors.cep[0] }}</small></label><label><span>UF *</span><input v-model="form.uf" maxlength="2" placeholder="UF"></label><label class="field-span-2"><span>Logradouro *</span><input v-model="form.logradouro" placeholder="Rua, avenida ou rodovia"></label><label><span>Número *</span><input v-model="form.numero" placeholder="Número"></label><label><span>Complemento</span><input v-model="form.complemento" placeholder="Sala, bloco, referência…"></label><label><span>Bairro *</span><input v-model="form.bairro" placeholder="Bairro"></label><label><span>Município *</span><input v-model="form.municipio" placeholder="Município"></label><label><span>Código IBGE</span><input v-model="form.codigo_municipio_ibge" placeholder="7 dígitos"></label></div>
          <div v-else class="settings-form"><div class="logo-upload field-span-2"><span class="logo-preview"><img v-if="logoPreview" :src="logoPreview" alt="Logo da empresa"><ImagePlus v-else :size="25" /></span><div><strong>Logotipo da empresa</strong><p>PNG, JPG ou WebP com até 2 MB.</p><label class="button button--ghost button--compact">Escolher imagem<input type="file" accept="image/png,image/jpeg,image/webp" hidden @change="chooseFile($event, 'logo')"></label><small v-if="logo">{{ logo.name }}</small></div></div></div>
        </div>
