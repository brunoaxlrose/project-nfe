                ...
                <form @submit.prevent="emitir" class="space-y-5">
                    ...
                    <section x-show="etapa === 2" x-cloak class="card">
                        ...
                        <div class="grid gap-x-5 gap-y-4 md:grid-cols-2">
                            ...
                            <label><span class="label">CPF / CNPJ</span><input x-model="cliente.documento" @input="formatarDocumentoInput" @input.debounce.500ms="consultarDocumento" inputmode="numeric" maxlength="18" required class="field" placeholder="CPF ou CNPJ"><small x-show="consultandoCnpj" x-cloak class="mt-1 block text-xs text-blue-600">Consultando CNPJ...</small></label>
                            <label><span class="label">Inscrição Estadual</span><input x-model="cliente.ie" class="field" placeholder="Opcional"></label>
                            ...
                            <label><span class="label">UF</span><input x-model="cliente.uf" maxlength="2" class="field" placeholder="SP"></label>
                            <label><span class="label">Município</span><input x-model="cliente.cidade" required class="field"></label>
                            <label><span class="label">Bairro</span><input x-model="cliente.bairro" required class="field"></label>
                            <label class="md:col-span-2"><span class="label">Endereço</span><input x-model="cliente.logradouro" required class="field"></label>
                            <label><span class="label">Número</span><input x-model="cliente.numero" required class="field"></label>
                            <label><span class="label">Complemento</span><input x-model="cliente.complemento" class="field"></label>
                            <label><span class="label">Fone</span><input x-model="cliente.fone" class="field"></label>
                            <label><span class="label">E-mail</span><input x-model="cliente.email" type="email" class="field"></label>
                    ...
                    <section x-show="etapa === 5" x-cloak class="space-y-5">
                        ...
                        <div class="card"><label><span class="label">Informações complementares</span><textarea x-model="informacoesComplementares" rows="4" maxlength="2000" class="field" placeholder="Ex.: Remessa para industrialização - OP 123."></textarea></label></div>
                    </section>
                    ...
                    <div class="flex items-center justify-between border-t border-slate-200 pt-5"><button type="button" @click="voltar" x-show="etapa > 1" x-cloak class="border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">Voltar</button><span x-show="etapa === 1" x-cloak></span><button type="button" @click="avancar" x-show="etapa < 5" x-cloak class="ml-auto bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-700">Continuar <span aria-hidden="true">→</span></button><button type="submit" x-show="etapa === 5" x-cloak :disabled="isLoading" class="ml-auto inline-flex items-center gap-2 bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"><span x-show="isLoading" x-cloak class="h-4 w-4 animate-spin rounded-full border-2 border-blue-200 border-t-white"></span><span x-text="isLoading ? (simulacaoHabilitada ? 'Simulando emissão...' : 'Enviando para a SEFAZ...') : (simulacaoHabilitada ? 'Simular emissão' : 'Emitir nota fiscal')"></span></button></div>
                </form>
            </main>
    ...
    <script>
        if (!window.fiscalFormNotaRegistered) {
            ...
            window.registerFiscalAlpine((Alpine) => {
                ...
                async buscarDestinatarios() { const termo = this.cliente.razao_social.trim(); if (termo.length < 2) { this.destinatario = []; return; } try { const [clientesResponse, destinatariosResponse] = await Promise.all([fiscalFetch('/api/clientes/buscar?q=' + encodeURIComponent(termo), { headers: this.headers() }), fiscalFetch('/api/destinatarios/buscar?q=' + encodeURIComponent(termo), { headers: this.headers() })]); const cliente = clientesResponse.ok ? (await clientesResponse.json()).map((item) => ({ ...item, origem: 'cliente' })) : []; const parceiros = destinatariosResponse.ok ? (await destinatariosResponse.json()).map((item) => ({ ...item, origem: 'destinatario' })) : []; this.destinatario = [...cliente, ...parceiros].slice(0, 10); this.mostrarDestinatarios = true; } catch (error) { if (!error.authExpired) this.notify('warning', 'A busca local está indisponível. Preencha o destinatário manualmente.', 'Busca local'); } },
                selecionarDestinatario(item) { this.cliente = { id: item.id, origem: item.origem, razao_social: item.razao_social || item.nome_razao_social, documento: this.formatarDocumento(item.documento), ie: item.inscricao_estadual || '', cep: item.cep || '', logradouro: item.logradouro || '', numero: item.numero || '', complemento: item.complemento || '', bairro: item.bairro || '', cidade: item.cidade || item.municipio || '', uf: item.uf || '', codigo_ibge: item.codigo_ibge || item.codigo_municipio_ibge || '', fone: item.fone || '', email: item.email || '' }; this.destinatario = []; this.mostrarDestinatarios = false; },
                ...
                async buscarProdutos(produto) { const termo = (produto.descricao || produto.codigo || '').trim(); if (termo.length < 2) { produto.sugestoes = []; return; } try { const response = await fiscalFetch('/api/produtos/buscar?q=' + encodeURIComponent(termo), { headers: this.headers() }); produto.sugestoes = response.ok ? await response.json() : []; } catch (error) { produto.sugestoes = []; } },
                    ...
                    try {
                        const response = await fiscalFetch('/api/nfe', {
                            ...
                            body: JSON.stringify({
                                ...
                                serie: Number(this.form.serie),
                                id_natureza_operacao: Number(this.form.id_natureza_operacao),
                                id_cliente: this.cliente.origem === 'cliente' ? this.cliente.id : null,
                                id_destinatario: this.cliente.origem === 'destinatario' ? this.cliente.id : null,
                                tipo_saida: this.form.tipo_saida,
                                data_emissao: this.form.data_emissao,
                                hora_emissao: this.form.hora_emissao,
                                data_saida: this.form.data_saida,
                                hora_saida: this.form.hora_saida,
                                finalidade: this.form.finalidade,
                                ind_pres: Number(this.form.ind_pres),
                                consumidor_final: this.form.consumidor_final,
                                produtos: this.produtos,
                                desconto: Number(this.totais.desconto || 0),
                                outras_despesas: Number(this.totais.outras_despesas || 0),
                                calculo_automatico: this.form.calculo_automatico,
