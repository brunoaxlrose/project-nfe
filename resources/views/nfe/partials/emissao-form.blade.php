    ...
    <script>
        if (!window.fiscalFormNotaRegistered) {
            ...
            window.registerFiscalAlpine((Alpine) => {
                ...
                selecionarNatureza(item) {
                    this.form.id_natureza_operacao = item.id;
                    this.naturezaBusca = item.descricao || item.nome;
                    this.mostrarNaturezas = false;
                    if (!this.informacoesComplementares && item.informacoes_complementares) {
                        this.informacoesComplementares = item.informacoes_complementares;
                    }
                    this.produtos.forEach((produto) => {
                        produto.cfop = item.cfop_padrao || produto.cfop;
                        produto.csosn = item.csosn_padrao || produto.csosn;
                    });
                },
